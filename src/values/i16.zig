...
pub const ValueI16 = struct {
    ...
    pub fn show(self: *const ValueI16, writer: *std.io.Writer, end: u8) !void {
        if (self.value) |val| {
            try writer.print("i16<{}>{c}", .{ val, end });
        } else {
            try writer.print("i16<null>{c}", .{end});
        }
    }
    ...
    pub fn getKind(_: *const ValueI16) ValueKind {
