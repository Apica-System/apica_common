...
pub const ValueI64 = struct {
    ...
    pub fn show(self: *const ValueI64, writer: *std.io.Writer, end: u8) !void {
        if (self.value) |val| {
            try writer.print("i64<{}>{c}", .{ val, end });
        } else {
            try writer.print("i64<null>{c}", .{end});
        }
    }
    ...
    pub fn getKind(_: *const ValueI64) ValueKind {
